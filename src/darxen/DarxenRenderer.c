...
struct _DarxenRendererPrivate
{
	...
	SettingsSite *radarSite;

	int intWidth;
	int intHeight;

	gboolean blnInit;
	gboolean blnSizeChanged;
	gboolean blnDataChanged;

	float fltRadiusBase;
	float fltRadius;

	GSList* products; /* ProductInfo */
...
darxen_renderer_init(DarxenRenderer *renderer)
{
	...
	priv->objData				= NULL;
	priv->viewSettings			= NULL;
	priv->radarSite				= NULL;

	priv->intWidth				= 0;
	priv->intHeight				= 0;

	priv->blnSizeChanged		= TRUE;
	priv->blnDataChanged		= TRUE;
...
GObject*
darxen_renderer_new(SettingsView *viewSettings, SettingsSite *site)
{
	...
	priv->viewSettings = viewSettings;
	priv->radarSite = site;
...
darxen_renderer_render_internal(DarxenRenderer *renderer)
{
	...
	glScalef(renderSettings->fltScale, renderSettings->fltScale, 1.0);
	glTranslatef(renderSettings->fltOffsetX, renderSettings->fltOffsetY, 0.0f);

	darxen_renderer_render_underlay(renderer);

	if (!priv->blnDataChanged && priv->viewSettings->intDispListProductsBase)
	{
		glCallList(priv->viewSettings->intDispListProductsBase);
	}
	else
	{
		if (priv->viewSettings->intDispListProductsBase)
			glDeleteLists(priv->viewSettings->intDispListProductsBase, 1);
		priv->viewSettings->intDispListProductsBase = glGenLists(1);
		glNewList(priv->viewSettings->intDispListProductsBase, GL_COMPILE_AND_EXECUTE);
			if (priv->viewSettings->viewstyle == VIEWSTYLE_SINGLE)
			{
				darxen_renderer_render_radial_data(renderer, &priv->objData->objDescription.objSymbologyBlock->objPackets[0].objRadialData);
			}
			else if (priv->viewSettings->viewstyle == VIEWSTYLE_QUADRUPLE)
			{
				g_critical("QUADRUPLE not implemented");
			}
		glEndList();
		priv->blnDataChanged = FALSE;
	}
...
//	SettingsView* view = priv->viewSettings;
//	SettingsViewProduct* product = (SettingsViewProduct*)view->products->data;
...
//		glOrtho(-priv->fltRadius, priv->fltRadius, -priv->fltRadius, priv->fltRadius, -1, 1);
...
//						darxen_rendering_common_free_font(font);
//						free(font);
...
darxen_renderer_render_radial_data(DarxenRenderer *renderer, ProductsLevel3RadialDataPacket *objData)
{
	...
	objPalette = settings_get_main()->renderer.objPaletteReflectivity;
	...
	SettingsView* view = priv->viewSettings;
	if (((SettingsViewProduct*)view->products->data)->smoothing) //we should be passed the settingsViewProduct, this can't work with quad
	{
...
//		gluNurbsCallback(nurb, GLU_NURBS_BEGIN, nurbsBegin);
//		gluNurbsCallback(nurb, GLU_NURBS_VERTEX, nurbsVertex);
//		gluNurbsCallback(nurb, GLU_NURBS_ERROR, nurbsVertex);
		...
		glBegin(GL_LINE_LOOP);
		{
			...
			{
				fltStart = 90.0 - objData->objRadials[az].fltStartAngle;
